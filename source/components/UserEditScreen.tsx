...
const UserEditScreen: React.FC<UserEditScreenProps> = ({ user, onRouteChange }) => {
	...
	const [focusedField, setFocusedField] = useState<'name' | 'email' | 'age' | 'department' | 'experience' | null>('name');
	...
	const moveToNextField = (currentField: keyof typeof formData) => {
		const fields: Array<keyof typeof formData> = ['name', 'email', 'age', 'department', 'experience'];
	...
	return (
		<Box flexDirection="column" padding={2}>
			<Box marginBottom={1}>
				<Text bold color="cyan">编辑用户信息</Text>
			</Box>
