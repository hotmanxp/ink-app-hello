...
const UserListScreen: React.FC<UserListScreenProps> = ({ onRouteChange }) => {
	...
	const getUserMenuItems = (): UserMenuItem[] => {
		...
		if (deleteConfirmation && selectedUserId) {
			...
			if (userToDelete) {
				return [
					{ label: `确认删除用户 ${userToDelete.name} (${userToDelete.email})?`, value: 'info', disabled: true },
		...
		if (showUserActions && selectedUserId) {
			...
			if (selectedUser) {
				return [
					{ label: `用户: ${selectedUser.name} (${selectedUser.email})`, value: 'info', disabled: true },
	...
	return (
		<Box flexDirection="column" padding={2}>
			<Box marginBottom={1}>
				<Text bold color="cyan">用户列表</Text>
			</Box>

			{deleteStatus && (
				<Box marginBottom={1}>
					<Text color={deleteStatus.success ? 'green' : 'red'}>
